...
for file in os.listdir(dir):
    ...
    write_path = "/Users/Jeff/Desktop/svm_hmm/input_16/" + str(file_name) + ".txt"
    ...
    for i in range(int(F.shape[0])):
        ...
        if i-3 in range(int(F.shape[0])):
            multiple_vec = list(F[i]) + list(F[i-1]) + list(F[i-2]) + list(F[i-3])
        elif i-2 in range(int(F.shape[0])):
        ...
        for j in range(len(multiple_vec)):
            for k in range(j+1):
                T.append(multiple_vec[j]*multiple_vec[k]*multiple_vec[k])

        for t in range(len(T)):
            text_file.write((" "+str(t+49))+":"+str(T[t]))
