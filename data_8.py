...
for file in os.listdir(dir):
    ...
    write_path = "/Users/Jeff/Desktop/svm_hmm/input_8/" + str(file_name) + ".txt"
    ...
    for i in range(int(F.shape[0])):
        ...
        for j in range(int(F.shape[1])):
            if i-1 in range(int(F.shape[0])):
                text_file.write(" "+str(j+1+3*int(F.shape[1]))+":"+str(F[i-1, j]))

        for j in range(int(F.shape[1])):
            if i-2 in range(int(F.shape[0])):
                text_file.write(" "+str(j+1+4*int(F.shape[1]))+":"+str(F[i-2, j]))
